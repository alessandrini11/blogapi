        ...
        <div class="w-11/12 mx-auto">
            ...
            <table class="min-w-max w-full table-auto">
                ...
                <tr v-for="(post) in posts" :key="post.id" class="">
                    ...
                    <td class="p-1">
                        <div class="h-10">
                            <img class="max-w-full h-full"  :src="post.img" alt="">
                        </div>
                    </td>
                    <td>{{post.title}}</td>
                    ...
                    <td>
                        <div class="flex justify-center">
                            <a class="" :href="articleViewUrl+`/${post.id}`"><i class="fa fa-eye hover:text-yellow-300" aria-hidden="true"></i></a>
                            <a @click.prevent="getEditForm(post.id)" class="mx-1" href=""><i class="fas fa-pencil-alt hover:text-green-500" aria-hidden="true"></i></a>
                ...
                <div class="flex justify-center items-center">
                    <form class="w-max bg-white p-2" @submit.prevent="createArticle">
                        ...
                        <div class="flex flex-col my-2">
                            ...
                            <textarea v-model="content" class="border-2 rounded-md py-1 px-4 "  cols="10" rows="10"></textarea>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div class="flex flex-col my-2">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="flex flex-col my-2">
                                ...
                                <select multiple class="border-2 rounded-md py-2 px-4" name="" >
                                    <option v-for="(categor,key) in categories" v-model="category" :value="categor[`@id`]">{{ categor.nom }}</option>
                                </select>
                            </div>
                            <div class="flex flex-col my-2">
                                <label for="">Author</label>
                                <select name="" class="rounded-md border-2 my-2 px-4">
                                    <option v-model="userPost" v-for="(user,key) in users"  :value="user[`@id`]">{{ user.nom}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="flex flex-col my-2">
            ...
            <div :class="{block : editModal, hidden : !editModal}" class="bg-gray-100 w-full h-full fixed left-0 top-0 flex justify-center items-center flex-col ">
                <div class="flex justify-center items-center">
                    <form class="w-max bg-white p-2" @submit.prevent="editArticle(postId)">
                        <div class="flex justify-between">
                            <h3 class="text-xl font-bold">Edit post</h3>
                            <p class="text-right"><button @click.prevent="editModal = !editModal" class="text-xl"><i class="fas fa-times"></i></button></p>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="flex flex-col my-2">
                                <label for="">Catégories</label>
                                <select multiple class="border-2 rounded-md py-2 px-4" name="" id="">
                                    <option v-for="(categor,key) in categories" v-model="category" :value="categor['@id']">{{ categor.nom }}</option>
                                </select>
                            </div>
                            <div class="flex flex-col my-2">
                                <label for="">Author</label>
                                <select name="" id="" class="rounded-md border-2 my-2 px-4">
                                    <option v-model="userPost"  :value="getUser['@id']">{{ getUser.nom}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="flex flex-col my-2">
...
<script>
    ...
    export default {
        ...
        data : () => {
            return {
                articleViewUrl : `http://127.0.0.1:8000/admin/article`,
                posts : [],
                users : [],
                categories : [],
                getUser : {},
        ...
        methods : {
            ...
            editArticle(id){
                ...
                axios.put(`/admin/api/articles/${id}`,thePost)
                    .then(response => this.postId = response.data.id)
                    .catch(err => console.log(err))
                this.editModal = false
                alert("Votre article a été bien modifié")
            }
